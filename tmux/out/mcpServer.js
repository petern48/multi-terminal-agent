...
- To modify a remote file: generate a unified diff (git diff format with a/ b/ prefixes) and call patch_file with the terminal name, filepath, and diff. Never overwrite remote files in their entirety with write_file or scp. Use write_file only for new files or local files.
    `.trim();
        ...
        server.tool("run_command", "Run a command in a named terminal. By default blocks until the command exits and returns output + exit code. Use background=true to fire-and-forget — the command runs in the pane foreground and can be stopped later with send_input C-c.", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        {
            name: zod_1.z.string(),
